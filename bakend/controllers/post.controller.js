...
export const deletePost = async (req, res) => {
    try {
        ...
        if(post.user.toString() !== req.user._id.toString()) {
            return res.status(401).json({err0t: "You are not autherized to delete this post"})
        }
        if(post.img) {
...
export const commentOnPost = async (req, res) => {
    try {
        ...
        console.log("Error in commitOnPost: ", error)
        res.status(500).json({error: "Internal server error"})        
    }    
}
...
export const likeUnlikePost = async (req, res) => {
    try {
        ...
        if(userLikedPost){
            ...
            res.status(200).json({message: "Post unlked successfull"})
        } else {
            //Like Post
        ...
        console.log("Error in likedUnliked Post", error)
        res.status(500).json({error: "Internal Server Error" })
    }
}
